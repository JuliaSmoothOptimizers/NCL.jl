...
function test_NCLSolve(
  test::Bool;
  HS_begin_KKT::Int64 = 1,
  HS_end_KKT::Int64 = 8,
  HS_begin_NCL::Int64 = 1,
  HS_end_NCL::Int64 = 13,
)::Test.DefaultTestSet
  # Test parameters
  print_level_NCL = 0
  ...
  η = 0.0001
  ϵ = 0.0001
  probs_KKT = ["HS" * string(i) for i = HS_begin_KKT:HS_end_KKT]
  probs_NCL = ["HS" * string(i) for i = HS_begin_NCL:HS_end_NCL] #[13,15,17,19,20]
  ...
  @testset "NCLSolve NLP (only linear residuals)" begin
    @testset "KKT_check function" begin
      @testset "KKT_check(nlp) via ipopt" begin
        ...
        resol_nlp_ipopt = NLPModelsIpopt.ipopt(
          nlp;
          print_level = 0,
          tol = ω,
          constr_viol_tol = η,
      ...
      @testset "KKT_check(ncl_nlin_res) via ipopt" begin
        ...
        resol_ncl_ipopt = NLPModelsIpopt.ipopt(
          ncl_nlin_res;
          print_level = 0,
          tol = ω,
          constr_viol_tol = η,
    ...
    @testset "KKT_check(nlp) via NCLSolve" begin
      ...
      D = KKT_check(
        nlp,
        x_ncl[1:(ncl_nlin_res.nx)],
        λ_ncl,
        z_U_ncl[1:(ncl_nlin_res.nx)],
        z_L_ncl[1:(ncl_nlin_res.nx)],
      )
      @test D["optimal"]
      @test D["acceptable"]
    end
  ...
  @testset "NCLSolve NLP (all residuals)" begin
    ...
    @testset "KKT_check(nlp) via NCLSolve" begin
      ...
      D = KKT_check(
        nlp,
        x_ncl[1:(nlc_cons_res.nx)],
        λ_ncl,
        z_U_ncl[1:(nlc_cons_res.nx)],
        z_L_ncl[1:(nlc_cons_res.nx)],
      )
      @test D["optimal"]
      @test D["acceptable"]
    end
  ...
  @testset "NCLSolve HS (only linear residuals)" begin
    for name in probs_NCL # several tests
      ...
      @testset "$test_name" begin
        @test NCLSolve(nlp; linear_residuals = false).solver_specific[:internal_msg] ==
              Symbol("Solve_Succeeded")
      end
      finalize(nlp)
    end
  end
  ...
  @testset "NCLSolve HS (all residuals)" begin
    for name in probs_NCL # several tests
      ...
      test_name = name * " problem resolution"
      @testset "$test_name" begin
        @test NCLSolve(nlp; linear_residuals = true).solver_specific[:internal_msg] ==
              Symbol("Solve_Succeeded")
      end
      finalize(nlp)
    end
